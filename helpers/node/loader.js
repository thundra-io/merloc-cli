...
function _resolveFunction(object, nestedProperty) {
...
function _hasFolderPackageJsonTypeModule(folder) {
    ...
    if (fs.existsSync(pj)) {
        try {
            ...
            console.warn(
                '[MERLOC]', 'WARN', '-', new Date().toISOString(), '|',
...
/**
 * Loads user handler by given handler {@param fullHandlerString}
...
module.exports.loadUserHandler = async function(appRoot, fullHandlerString) {
    ...
    let handlerFunc = _resolveFunction(userApp, handlerPath);
    if (!handlerFunc) {
...
/**
 * Loads user init function by given handler {@param fullHandlerString}
 * from given path {@param appRoot}
 * @param {string} appRoot the application root path
 * @param {string} fullHandlerString the handler name including its module name
 * @param {string} initFunctionName the init function name
 * @return the loaded user init function
 */
module.exports.loadUserInit = async function(appRoot, fullHandlerString, initFunctionName = 'init') {
    _throwIfInvalidHandler(fullHandlerString);
    let [moduleRoot, moduleAndHandler] = _moduleRootAndHandler(fullHandlerString);
    let [module] = _splitHandlerString(moduleAndHandler);
    let userApp = await _loadUserApp(appRoot, moduleRoot, module);
    let initFunc = _resolveFunction(userApp, initFunctionName);
    if (initFunc && typeof initFunc !== 'function') {
        throw new InvalidHandler(`${module}.init is not a function`);
    }
    return initFunc;
};
